...
type AccountFormAccountIdProps = {
    accountId: string;
    ...
    type:string;
    disabled: boolean;
    localAlert: LocalAlertType; 
    clearLocalAlert: ()=> void;
    checkAvailability: (accountId: string)=> boolean;
    mainLoader: boolean;
}

class AccountFormAccountId extends Component<AccountFormAccountIdProps> {
    ...
    canvas = null;
    suffix:React.RefObject<HTMLSpanElement> = createRef();
    ...
    render() {
        ...
        const success = (localAlert as LocalAlertType)?.success ;
        const problem = !(localAlert as LocalAlertType)?.success && (localAlert as LocalAlertType)?.show;
                    ...
                    {({ translate }) => (
                        <InputWrapper className={classNames([type, {'success': success}, {'problem': problem}, {'wrong-char': wrongChar}])}>
                            <input
                                ...
                                value={accountId}
                                onInput={(e: React.ChangeEvent<HTMLInputElement>) => type === 'create' && this.updateSuffix(e.target.value.trim())}
                                onChange={(e: React.ChangeEvent<HTMLInputElement>) => this.handleChangeAccountId({ userValue: e.target.value.trim(), el: e.target })}
