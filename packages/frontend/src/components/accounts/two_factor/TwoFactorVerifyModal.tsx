...
type TwoFactorVerifyModalProps = {
    open: boolean;
    onClose: (verified: boolean, error: Error)=> void;  
}

const TwoFactorVerifyModal = ({ open, onClose }:TwoFactorVerifyModalProps) => {

    const [method, setMethod] = useState();
    const [code, setCode] = useState('');
    const [resendCode, setResendCode] = useState<string | null>();
    ...
    const handleResendCode = async () => {
        ...
        await Mixpanel.withTracking('2FA Modal Resend code', 
            async () => {
                ...
                setTimeout(() => { setResendCode(null); }, 3000);
            },
            (e: ErrorType) => {
    ...
    return (
        ...
        >
            ...
            <p className='color-black font-bw' style={{ marginTop: '-10px', fontWeight: '500', height: '19px'}}>{method && (method as MethodType).detail}</p>
