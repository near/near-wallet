...
type RecoverAccountSeedPhraseProps = {
    ...
    recoverAccountSeedPhrase: (seedPhrase: string)=> void;
    refreshAccount: ()=> void;
    localAlert: {
        show: string | boolean;
...
class RecoverAccountSeedPhrase extends Component<RecoverAccountSeedPhraseProps> {
    state = {
    ...
    handleSubmit = async () => {
        ...
        if (fundWithExistingAccount) {
            const createNewAccountParams = stringify(JSON.parse(fundWithExistingAccount as string));
            redirectTo(`/fund-with-existing-account?${createNewAccountParams}`);
        } else {
            const options = parseFundingOptions(location.search);
            if (options) {
                const query = parseQuery(location.search);
                const redirectUrl = query.redirectUrl ? `?redirectUrl=${encodeURIComponent(query.redirectUrl as string)}` : '';
